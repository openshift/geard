...
func RegisterInit(parent *cobra.Command) {
	initGearCmd := &cobra.Command{
		Use:   "init <name> <image>",
		Short: "(DEPRECATED) Setup the environment for a container",
		Long:  "Replaced by init-container and link-container",
		Run:   initGear,
	}
...
func initPreStart(dockerSocket string, id containers.Identifier, imageName string) error {
	...
	containerData := isolateInitScript{
		imgInfo.Config.User == "",
		user,
		u.Uid,
		u.Gid,
	...
	if erre := isolateInitTemplate.Execute(file, containerData); erre != nil {
	...
	if erre := isolateCmdTemplate.Execute(file, containerData); erre != nil {
